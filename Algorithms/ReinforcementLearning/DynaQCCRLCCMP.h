...
class DynaQCCRLCCMP: public DynaQCCBase<S, A>,
		public ReinforcementLearning<S, A> {
...
inline AI::Algorithm::DynaQCCRLCCMP<S, A>::DynaQCCRLCCMP(AI::FLOAT stepSize,
		...
		AI::FLOAT stateTransitionStepSize) :
		ReinforcementLearning<S, A>(stepSize, discountRate, policy), DynaQCCBase<
				S, A>(simulationIterationCount, stateTransitionGreediness,
...
inline void AI::Algorithm::DynaQCCRLCCMP<S, A>::backUpStateActionPair(
		const StateAction<S, A>& currentStateAction, const FLOAT reward,
		const StateAction<S, A>& nextStateActionPair) {
	ReinforcementLearning<S, A>::backUpStateActionPair(currentStateAction,
			reward, nextStateActionPair);
}
