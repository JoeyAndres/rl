...
namespace AI {
  ...
  void StateActionTransition<S>::update(const S& nextState,
	...
	for (auto iter = _stateActionTransitionFrequency.begin();
	  ...
	  if (state != nextState) {
		_stateActionTransitionFrequency[state] =
			_stateActionTransitionFrequency[state] + _stepSize
				* (0.0F - _stateActionTransitionFrequency[state]);
	  }
	}

	// --Increase the value of the next frequency.
	_stateActionTransitionFrequency[nextState] =
		_stateActionTransitionFrequency[nextState] + _stepSize
	...
	_stateActionTransitionReward[nextState] =
		_stateActionTransitionReward[nextState] + 1.0F
			* (reward - _stateActionTransitionReward[nextState]);

	assert(
		_stateActionTransitionFrequency.size() == _stateActionTransitionReward.size());
  }
