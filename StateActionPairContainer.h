...
namespace AI {
  ...
  public:
	 ...
	 * @param actionSet
	 */
	virtual void addAction(A& action, AI::FLOAT value,
		const set<S>& actionSet);
	...
	virtual void setStateActionValue(const StateAction<S, A>& stateAction,
		AI::FLOAT value) throw (StateActionNotExistException);

	const AI::FLOAT& operator[](
		const StateAction<S, A>& stateAction) const;
  ...
  template<class S, class A>
  void StateActionPairContainer<S, A>::addState(const S& state,
	  AI::FLOAT value, const set<A>& actionSet) {
  ...
  void StateActionPairContainer<S, A>::addAction(A& action, AI::FLOAT value,
	...
	for (const S& state : stateSet) {
	  _stateActionPairMap.insert(
		  std::pair<StateAction<S, A>, AI::FLOAT >(StateAction<S, A>(state, action),
			  value));
	}
  }
  ...
  template<class S, class A>
  bool StateActionPairContainer<S, A>::stateInStateActionPairMap(const S& state,
	  const set<A>& actionSet) const {
