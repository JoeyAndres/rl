...
ReinforcementLearningGD::ReinforcementLearningGD(TileCode& tileCode,
		AI::FLOAT stepSize, AI::FLOAT discountRate, AI::FLOAT lambda,
		Policy<vector<FLOAT>, vector<FLOAT> >& policy) :
		LearningAlgorithm<vector<FLOAT>, vector<FLOAT> >(policy), _gradientDescent(
...
inline void ReinforcementLearningGD::update(
	...
	_gradientDescent.updateWeights(currentStateAction.getState(),
			currentStateAction.getAction(), nextStateVector,
			actionValueMap[nextAction], reward);
}
