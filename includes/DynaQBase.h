...
inline AI::Algorithm::DynaQBase<S, A>::DynaQBase(
		...
		AI::FLOAT stateTransitionStepSize) :
		_simulationIterationCount(simulationIterationCount), _stateActionTransitionGreediness(
				stateTransitionGreediness), _stateActionTransitionStepSize(
...
inline void AI::Algorithm::DynaQBase<S, A>::_addModel(
		const StateAction<S, A>& currentStateAction) {
	_model.emplace(std::piecewise_construct,
