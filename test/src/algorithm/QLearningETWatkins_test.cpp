...
using rl::algorithm::QLearningETFactory;
...
SCENARIO("QLearningETWatkins converge to a solution",
  ...
  GIVEN("A random walk environment") {
    ...
    auto qLearningETWatkins =
      QLearningETFactory<rl::INT, rl::INT>(0.1F, 0.9F, policy, 0.5).get();

    rl::agent::Agent <rl::INT, rl::INT> agent(rwe, qLearningETWatkins);
