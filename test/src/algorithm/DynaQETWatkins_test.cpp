...
using rl::algorithm::DynaQETFactory;
...
SCENARIO("DynaQTETWatkins converge to a solution",
  ...
  GIVEN("A random walk environment") {
    ...
    auto dynaQETWatkins = DynaQETFactory<rl::INT, rl::INT>(
      0.1F, 0.9F, policy, 50, 1.0F, 1.0F, 0.5F).get();

    rl::agent::Agent <rl::INT, rl::INT> agent(rwe, dynaQETWatkins);
