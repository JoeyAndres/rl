...
void ReplaceChar(string& str, char c, string const& replacement) {
  for (size_t pos = str.find(c); pos != string::npos;
...
string BuildFailureMessage(string const& file, int line,
...
void XmlTestReporter::ReportSummary(int totalTestCount, int failedTestCount,
  ...
  BeginResults(m_ostream, totalTestCount, failedTestCount, failureCount,
               secondsElapsed);
...
void XmlTestReporter::BeginResults(std::ostream& os, int totalTestCount,
                                   int failedTestCount, int failureCount,
                                   float secondsElapsed) {
  os << "<unittest-results" << " tests=\"" << totalTestCount << "\""
      << " failedtests=\"" << failedTestCount << "\"" << " failures=\""
      << failureCount << "\"" << " time=\"" << secondsElapsed << "\"" << ">";
}
...
void XmlTestReporter::BeginTest(std::ostream& os,
  ...
  os << "<test" << " suite=\"" << result.suiteName << "\"" << " name=\""
      << result.testName << "\"" << " time=\"" << result.timeElapsed << "\"";
}
...
void XmlTestReporter::AddFailure(std::ostream& os,
  ...
  for (DeferredTestResult::FailureVec::const_iterator it =
      result.failures.begin(); it != result.failures.end(); ++it) {
    string const escapedMessage = XmlEscape(it->second);
    string const message = BuildFailureMessage(result.failureFile, it->first,
                                               escapedMessage);
