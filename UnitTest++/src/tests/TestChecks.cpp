...
TEST(CheckArrayCloseTrue) {
	...
	CheckArrayClose(results, array1, array2, 3, 0.02f,
			TestDetails("", "", "", 0));
	CHECK_EQUAL(0, results.GetFailureCount());
}
...
TEST(CheckArrayCloseFalse) {
	...
	float const array1[3] = { 1.0f, 1.5f, 2.0f };
	float const array2[3] = { 1.01f, 1.51f, 2.01f };
	CheckArrayClose(results, array1, array2, 3, 0.001f,
			TestDetails("", "", "", 0));
	CHECK_EQUAL(1, results.GetFailureCount());
}
...
TEST(CheckArrayCloseFailureIncludesDetails) {
	RecordingReporter reporter;
	TestResults results(&reporter);
	TestDetails const details("arrayCloseTest", "arrayCloseSuite", "file",
			1337);
...
TEST(CheckArray2DCloseTrue) {
	...
	float const array1[3][3] = { { 1.0f, 1.5f, 2.0f }, { 2.0f, 2.5f, 3.0f }, {
			3.0f, 3.5f, 4.0f } };
	float const array2[3][3] = { { 1.01f, 1.51f, 2.01f },
			{ 2.01f, 2.51f, 3.01f }, { 3.01f, 3.51f, 4.01f } };
	CheckArray2DClose(results, array1, array2, 3, 3, 0.02f,
			TestDetails("", "", "", 0));
	CHECK_EQUAL(0, results.GetFailureCount());
}
...
TEST(CheckArray2DCloseFalse) {
	...
	float const array2[3][3] = { { 1.01f, 1.51f, 2.01f },
			{ 2.01f, 2.51f, 3.01f }, { 3.01f, 3.51f, 4.01f } };
	CheckArray2DClose(results, array1, array2, 3, 3, 0.001f,
			TestDetails("", "", "", 0));
	CHECK_EQUAL(1, results.GetFailureCount());
}
...
TEST(CheckArray2DCloseFailureIncludesDetails) {
	RecordingReporter reporter;
	TestResults results(&reporter);
	TestDetails const details("array2DCloseTest", "array2DCloseSuite", "file",
			1234);
