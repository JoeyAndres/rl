...
TEST_FIXTURE(TestRunnerFixture, TestFinishIsCalledWithCorrectTime) {
	...
	runner.RunTestsIf(list, NULL, True(), 0);
	CHECK(
			reporter.lastFinishedTestTime >= 0.005f
					&& reporter.lastFinishedTestTime <= 0.050f);
}
...
struct TestSuiteFixture {
	TestSuiteFixture() :
			test1("TestInDefaultSuite"), test2("TestInOtherSuite",
					"OtherSuite"), test3("SecondTestInDefaultSuite"), runner(
					reporter) {
		list.Add(&test1);
		list.Add(&test2);
	}
