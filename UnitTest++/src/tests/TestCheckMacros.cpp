...
TEST(CheckEqualFailureContainsCorrectDetails) {
  ...
  {
    UnitTest::TestResults testResults(&reporter);
    UnitTest::TestDetails const testDetails("testName", "suiteName", "filename",
                                            -1);
...
TEST(CheckCloseFailureBecauseOfExceptionContainsCorrectDetails) {
  ...
  {
    UnitTest::TestResults testResults(&reporter);
    UnitTest::TestDetails testDetails("closeTest", "closeSuite", "filename",
                                      -1);
    ScopedCurrentTest scopedResults(testResults, &testDetails);
    CHECK_CLOSE((float )ThrowingFunction(), 1.0001f, 0.1f);
    line = __LINE__;
...
TEST(CheckArrayCloseFailureContainsCorrectDetails) {
  ...
  {
    UnitTest::TestResults testResults(&reporter);
    UnitTest::TestDetails testDetails("arrayCloseTest", "arrayCloseSuite",
                                      "filename", -1);
    ScopedCurrentTest scopedResults(testResults, &testDetails);
...
TEST(CheckArrayCloseFailureBecauseOfExceptionContainsCorrectDetails) {
  ...
  RecordingReporter reporter;
  {
    ...
    CHECK_ARRAY_CLOSE(data, ThrowingObject(), 4, 0.01f);
    line = __LINE__;
  }
...
TEST(CheckArrayEqualFailureContainsCorrectInfo) {
  ...
  CHECK_EQUAL("CheckArrayEqualFailureContainsCorrectInfo",
              reporter.lastFailedTest);
...
TEST(CheckArray2DCloseFailureContainsCorrectDetails) {
  ...
  {
    UnitTest::TestResults testResults(&reporter);
    UnitTest::TestDetails testDetails("array2DCloseTest", "array2DCloseSuite",
                                      "filename", -1);
    ScopedCurrentTest scopedResults(testResults, &testDetails);
...
TEST(CheckArray2DCloseFailureBecauseOfExceptionContainsCorrectDetails) {
  ...
  RecordingReporter reporter;
  {
    ...
    const float data[2][2] = { { 0, 1 }, { 2, 3 } };
    CHECK_ARRAY2D_CLOSE(data, ThrowingObject2D(), 2, 2, 0.01f);
    line = __LINE__;
  }
